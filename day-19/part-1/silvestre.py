...
class SilvestreSubmission(Submission):
    ...
    def run(self, s):
        ...
        while curr_i >= 0 and curr_j >= 0 and curr_i <= max_i and curr_j <= max_j:
            direction, curr_i, curr_j, letters = self.execute_instructions(tubes, direction, curr_i, curr_j, letters)
        return "".join(letters)

    def execute_instructions(self, tubes, direction, curr_i, curr_j, letters):
        if direction == 0:
            curr_i += 1
        elif direction == 1:
            curr_j -= 1
        ...
        while not curr_c == "+":
            ...
            if not (curr_i, curr_j) in tubes:
                return 0, -1, -1, letters
            curr_c = tubes[(curr_i, curr_j)]
            if bool(re.match(r'^[A-Z]$',curr_c)):
                letters.append(curr_c)
        ...
        return direction, curr_i, curr_j, letters
